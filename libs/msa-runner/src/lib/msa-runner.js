...
var run = function () {
    var port = Number(dotenv.parse(fs.readFileSync('.env'))['MSA_APP_START_PORT']) || 5000;
    ...
    if (!isEnvExist(gateway))
        createEnv(gateway);
    updateGatewayEnv(gateway, Number(dotenv.parse(fs.readFileSync(".env"))['MSA_GATEWAY_PORT']) || 4000, mapProjectToPort);
